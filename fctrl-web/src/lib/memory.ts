...
const PROJECTS: Project[] = [
  {
    id: 'prj-travel',
    name: 'Travel Webapp',
    description: 'Booking flow, auth and payments.',
    createdAt: Date.now(),
  },
  {
    id: 'prj-beer',
    name: 'Beer App',
    description: 'Tasting notes and cellar tracking.',
    createdAt: Date.now(),
  },
  {
    id: 'prj-docs',
...
const NODES: FlowNode[] = [
  ...
  task(
    'T-1041',
    'WP-AUTH',
    'Session store on Redis with sliding expiry',
    'DONE',
    'redis-cli ping',
    'Replaces the in-process session map. Sliding expiry of 30m, hard cap 12h.',
    'pass',
    '3d ago',
  ),
  step(
  ...
  task(
    'T-1042',
    'WP-AUTH',
    'OAuth2 device-code flow for the CLI',
    'READY',
  ...
  step(
    'T-1042.1',
    'T-1042',
    'Register client credentials in provider',
    'DONE',
    'manual ✓',
  ),
  step(
    'T-1042.2',
    'T-1042',
    'Poll token endpoint with backoff',
    'DONE',
    'curl -sf /device/token',
  ),
  step(
    'T-1042.3',
    'T-1042',
    'Persist refresh token to OS keyring',
    'READY',
    'fctrl auth whoami',
  ),
  step(
    'T-1042.4',
    'T-1042',
    'Handle expired_token + slow_down',
    'BLOCKED',
    'pnpm test:auth --grep slowdown',
  ),
  step(
    'T-1042.5',
    'T-1042',
    'Docs: CLI login walkthrough',
    'BLOCKED',
    'file exists: docs/cli-login.md',
  ),
  ...
  task(
    'T-2010',
    'WP-BOOK',
    'Availability search across provider adapters',
    'READY',
    'pnpm test:booking',
  ...
  step(
    'T-2010.2',
    'T-2010',
    'Fan-out with timeout budget',
    'DONE',
    'pnpm test:booking --grep fanout',
  ),
  step(
    'T-2010.3',
    'T-2010',
  ...
  task(
    'T-2011',
    'WP-BOOK',
    'Hold-and-confirm two-phase reservation',
    'BLOCKED',
    'pnpm test:booking --grep hold',
  ...
  task(
    'T-2012',
    'WP-BOOK',
    'Idempotency keys on the confirm endpoint',
    'BLOCKED',
    'file exists: docs/idempotency.md',
  ...
  task(
    'T-4002',
    'WP-OBS',
    'Structured log schema for the Rust core',
    'READY',
    'cargo test --package fctrl-core log',
    'One event shape for the engine, the web app and the TUI.',
  ),
  task(
    'T-4000',
    'WP-OBS',
    'OTel collector bootstrap',
    'DONE',
    'kubectl get pods -l otel',
    '',
    'pass',
    '2w ago',
  ),
  ...
  },
  {
    ...
    parentId: 'WP-BEER',
    type: 'TASK',
    title: 'Bottle inventory model',
    status: 'READY',
    condition: 'pnpm test:cellar',
    lastResult: 'none',
  },
  {
    id: 'WP-DOCS',
...
export class MemoryStore implements FlowStore {
  ...
  async dependencies(projectId: string): Promise<Dependency[]> {
    await sleep(20);
    const ids = new Set(
      this.nodeList.filter((n) => n.projectId === projectId).map((n) => n.id),
    );
    return this.depList
      .filter((d) => ids.has(d.blockerId) || ids.has(d.blockedId))
      .map((d) => ({ ...d }));
  }
