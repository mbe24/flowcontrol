...
describe('RemoteStore', () => {
  ...
  it('updateNode sends a mask for only provided fields (note skipped)', async () => {
    const { client } = fakeClient();
    const store = storeWith(client);
    await store.updateNode('T-1042', { title: 'Renamed', note: 'body' });
    const req = (client.updateNode as ReturnType<typeof vi.fn>).mock.calls[0][0];
    expect(req.updateMask).toEqual(['title']);
  ...
  it('unbacked writes are no-ops that still resolve', async () => {
    const { client } = fakeClient();
    const store = storeWith(client);
    await store.moveNode('T-1042', 'WP-X', 'STEP');
    await store.createProject('P', 'd', true);
